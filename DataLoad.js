...
RecordsInserted["Publisher__c"] = new Map();
RecordsInserted["Listing__c"] = new Map();
RecordsInserted["SubscriptionPlan__c"] = new Map();
...
function deleteRecords() {
  var deletePublishers = deleteExistingRecordsBulk(conn, "Publisher__c");
  var deleteListings = deleteExistingRecordsBulk(conn, "Listing__c");
  var deletePlans = deleteExistingRecordsBulk(conn, "SubscriptionPlan__c");

  Promise.all([deletePublishers, deleteListings, deletePlans]).then(
    (response) => {
...
function loadFirstLevelObjects() {
  console.log(
    "\n" + new Date().toLocaleTimeString() + " --- DATA LOAD INIT ---"
  );
  console.log(
    new Date().toLocaleTimeString() + " --- Insert first level (Publishers) ---"
  );

  var publisherInsert = processCSVFileWithLookups(
    "./publisher.csv",
    "Publisher__c",
    conn,
    [],
  ...
  Promise.all([publisherInsert]).then((response) => {
    console.log(
      "\n" +
